...
function decodeScriptNum(bytes) {
  ...
  if ((top & 128) !== 0)
    return null;
  if (top === 0 && (bytes.length === 1 || (bytes[bytes.length - 2] & 128) === 0)) {
    return null;
  }
  let value = 0;
...
function parseHTLCScript(scriptBytes) {
  ...
  if (locktime === null || locktime < 1 || locktime >= 5e8)
    return null;
...
var MIN_DELTA_SECONDS = 12 * 3600;
var MAX_REF_STALENESS_BTC = 10;
var MAX_REF_STALENESS_FBC = 20;
var MIN_BLOCKS_TO_REFUND_BTC = 12;
var MIN_BLOCKS_TO_REFUND_FBC = 60;
var MAX_REFUND_SECONDS = 7 * 24 * 3600;
function fbcClaimDeadline(offer) {
  return offer.fbc_refund_height - CLAIM_SAFETY_BLOCKS_FBC;
}
var CLAIM_SAFETY_BLOCKS_FBC = 6;
function isHeight(v) {
  return Number.isInteger(v) && v >= 1 && v < 5e8;
}
function checkTimelocks(offer, observed) {
  const fields = {
  ...
  if (deltaSeconds < MIN_DELTA_SECONDS) {
    ...
    return {
      ok: false,
      reason: `unsafe timelocks: the BTC refund (T1, ${h(btcSecondsToT1)}h) must fall at least ${h(MIN_DELTA_SECONDS)}h after the FBC refund (T2, ${h(fbcSecondsToT2)}h); \u0394 = ${h(deltaSeconds)}h. Alice funds first and holds the preimage, so her refund must be last \u2014 otherwise she can refund her BTC and still claim the FBC (SPEC \xA74.2).`
    };
  }
  if (btcSecondsToT1 > MAX_REFUND_SECONDS || fbcSecondsToT2 > MAX_REFUND_SECONDS) {
    return {
      ok: false,
      reason: `refund height too far out: max ${MAX_REFUND_SECONDS / 86400} days (SPEC \xA74.3)`
    };
  }
  if (observed?.btcTip !== void 0 && observed.btcTip !== null) {
    ...
    if (Math.abs(drift) > MAX_REF_STALENESS_BTC) {
      return {
        ok: false,
        reason: `btc_reference_height ${offer.btc_reference_height} is ${Math.abs(drift)} blocks ${drift < 0 ? "behind" : "ahead of"} the observed tip ${observed.btcTip} (max ${MAX_REF_STALENESS_BTC}, SPEC \xA74.3)`
      };
    }
    if (offer.btc_refund_height < observed.btcTip + MIN_BLOCKS_TO_REFUND_BTC) {
      return {
        ok: false,
        reason: `btc_refund_height ${offer.btc_refund_height} is already live or nearly live against the observed tip ${observed.btcTip}`
      };
    }
  }
  if (observed?.fbcTip !== void 0 && observed.fbcTip !== null) {
    ...
    if (Math.abs(drift) > MAX_REF_STALENESS_FBC) {
      return {
        ok: false,
        reason: `fbc_reference_height ${offer.fbc_reference_height} is ${Math.abs(drift)} blocks ${drift < 0 ? "behind" : "ahead of"} the observed tip ${observed.fbcTip} (max ${MAX_REF_STALENESS_FBC}, SPEC \xA74.3)`
      };
    }
    if (offer.fbc_refund_height < observed.fbcTip + MIN_BLOCKS_TO_REFUND_FBC) {
      return {
        ok: false,
        reason: `fbc_refund_height ${offer.fbc_refund_height} is already live or nearly live against the observed tip ${observed.fbcTip}`
      };
    }
  }
  return { ok: true };
...
var BTC_NETWORKS = ["main", "testnet", "regtest"];
var FBC_NETWORKS = ["main", "testnet", "regtest", "simnet"];
...
function validateBlob(x) {
  ...
  switch (obj.kind) {
    ...
    case "funded_fbc":
      ...
      if (obj.htlc_address !== void 0 && typeof obj.htlc_address !== "string") {
...
function verifyFundedBtc(offer, accept, funded, observed) {
...
function verifyFundedFbc(offer, accept, funded, observed) {
  if (funded.offer_id !== offer.offer_id) {
    return { ok: false, reason: "offer_id mismatch" };
  }
...
async function blobQrDataUrl(text) {
  try {
    return await import_qrcode.default.toDataURL(text, {
