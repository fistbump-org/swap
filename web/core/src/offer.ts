 ...
 */
export function fbcClaimDeadline(offer: OfferBlob): number {
...
export type ObservedTips = { btcTip?: number | null; fbcTip?: number | null };
 ...
 * about to lock funds.
 */
export function checkTimelocks(offer: OfferBlob, observed?: ObservedTips): Check {
  const fields = {
  ...
  const btcSecondsToT1 =
    (offer.btc_refund_height - offer.btc_reference_height) * BTC_BLOCK_SECONDS;
  const fbcSecondsToT2 =
    (offer.fbc_refund_height - offer.fbc_reference_height) * FBC_BLOCK_SECONDS;
  ...
  if (deltaSeconds < MIN_DELTA_SECONDS) {
    ...
    return {
      ok: false,
      reason:
        `unsafe timelocks: the BTC refund (T1, ${h(btcSecondsToT1)}h) must fall at least ` +
  ...
  if (observed?.btcTip !== undefined && observed.btcTip !== null) {
    ...
    if (Math.abs(drift) > MAX_REF_STALENESS_BTC) {
      return {
        ok: false,
        reason:
          `btc_reference_height ${offer.btc_reference_height} is ${Math.abs(drift)} blocks ` +
    ...
    if (offer.btc_refund_height < observed.btcTip + MIN_BLOCKS_TO_REFUND_BTC) {
      return {
        ok: false,
        reason:
          `btc_refund_height ${offer.btc_refund_height} is already live or nearly live ` +
  ...
  if (observed?.fbcTip !== undefined && observed.fbcTip !== null) {
    ...
    if (Math.abs(drift) > MAX_REF_STALENESS_FBC) {
      return {
        ok: false,
        reason:
          `fbc_reference_height ${offer.fbc_reference_height} is ${Math.abs(drift)} blocks ` +
    ...
    if (offer.fbc_refund_height < observed.fbcTip + MIN_BLOCKS_TO_REFUND_FBC) {
      return {
        ok: false,
        reason:
          `fbc_refund_height ${offer.fbc_refund_height} is already live or nearly live ` +
...
const BTC_NETWORKS: readonly string[] = ["main", "testnet", "regtest"];
const FBC_NETWORKS: readonly string[] = ["main", "testnet", "regtest", "simnet"];

function fail(kind: string, field: string, why: string): never {
  throw new Error(`${kind} blob: ${field} ${why}`);
}

/** A hex string of exactly `bytes` bytes. Case-insensitive, matching `fromHex`. */
function hexField(obj: Record<string, unknown>, kind: string, field: string, bytes: number): void {
  const v = obj[field];
  if (typeof v !== "string") fail(kind, field, "must be a string");
  if (!new RegExp(`^[0-9a-fA-F]{${bytes * 2}}$`).test(v)) {
    fail(kind, field, `must be exactly ${bytes * 2} hex characters`);
  }
}

function pubkeyField(obj: Record<string, unknown>, kind: string, field: string): void {
  hexField(obj, kind, field, 33);
  if (!isCompressedPubkey(fromHex(obj[field] as string))) {
    fail(kind, field, "is not a valid compressed secp256k1 pubkey (02/03 prefix, on curve)");
  }
}

function nonNegIntField(obj: Record<string, unknown>, kind: string, field: string): void {
  const v = obj[field];
...
function heightField(obj: Record<string, unknown>, kind: string, field: string): void {
  if (!isHeight(obj[field])) fail(kind, field, "is not a valid block height");
}
 ...
 */
function scriptHexField(obj: Record<string, unknown>, kind: string, field: string): void {
  const v = obj[field];
...
function validateOffer(obj: Record<string, unknown>): OfferBlob {
  const net = obj.network;
  if (!net || typeof net !== "object") fail("offer", "network", "must be an object");
  const { btc, fbc } = net as Record<string, unknown>;
...
function validateAccept(obj: Record<string, unknown>): AcceptBlob {
...
function validateFunded(obj: Record<string, unknown>, kind: "funded_btc" | "funded_fbc"): void {
...
function validateBlob(x: unknown): SwapBlob {
  if (!x || typeof x !== "object" || Array.isArray(x)) throw new Error("blob is not an object");
 ...
 */
export function htlcParamsForRecovery(
  offer: OfferBlob,
  accept: AcceptBlob,
): { btc: HTLCParams; fbc: HTLCParams } {
...
export function verifyFundedFbc(
  ...
  if (funded.htlc_address) {
    const network = (offer.network?.fbc ?? "main") as FbcNetwork;
    let derived: string;
    try {
...
function isTxid(v: unknown): v is string {
